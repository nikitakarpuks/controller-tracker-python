"""
Fit max_area model:  max_area = a * facing_cos / depth_m² + b

Upper-bound fit: the curve is placed so that UPPER_PERCENTILE % of
non-outlier blobs fall BELOW it — i.e. no real LED blob gets filtered.
Outliers (oversized merged blobs) are excluded before fitting.
"""
...
UPPER_PCTILE  = 99.0   # % of clean data that must lie below the curve
...
below = (areas_c <= a * x_c + b).mean() * 100
print("=" * 56)
print(f"  max_area = {a:.5f} * cos / d²  +  {b:.5f}")
...
fig.suptitle(
    f"max_area = {a:.4f} · cos/d²  +  {b:.4f}   "
    f"({below:.0f}% below, {UPPER_PCTILE}th-pctile bound, n={clean.sum()})",
    fontsize=11,
)
...
ax.plot(x_line, a * x_line + b, color="tomato", linewidth=1.5, label=f"{UPPER_PCTILE}th pctile bound")
